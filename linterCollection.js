...
const generateResponseTests = (responseBody) => {
    const tests = [];
    let parsedBody;
...
const addTestsToRequest = (item, expectedStatusCode, responseBody) => {
    ...
    generateResponseTests(responseBody).forEach(test => {
...
const modifyCollection = (data, responseTime, version) => {
    try {
                ...
                if (item.item && Array.isArray(item.item)) {
                    ...
                    if (item.response && Array.isArray(item.response)) {
                        item.response.forEach((responseExample) => {
                            ...
                            addTestsToRequest(newItem, statusCode, responseExample.body);
...
app.post("/upload", upload.single("postmanCollection"), (req, res) => {
    if (!req.file) return res.render("index", { message: "Aucun fichier téléchargé !" });
    ...
    fs.readFile(filePath, "utf8", (err, data) => {
        if (err) return res.render("index", { message: "Erreur lors de la lecture du fichier." });

        try {
            const { jsonData, newCollectionName } = modifyCollection(data, responseTime, version);
            ...
            res.render("index", { message: `Erreur : ${error.message}` });
        }
    });
});
